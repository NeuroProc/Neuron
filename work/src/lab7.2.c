...
int main(int argc, char *argv[]) {
	...
	if (fcntl(TASK_RD, F_SETFL, O_NONBLOCK) < 0) {
		fprintf(stderr, "%d: %s: %s\n",getpid(), program, strerror(errno));
	}
	if (fcntl(EXEC_RD, F_SETFL, O_NONBLOCK) < 0) {
			fprintf(stderr, "%d: %s: %s\n",getpid(), program, strerror(errno));
		}
	processing(curDir);
	//sleep(100);
...
int ready(int fd, int flag) {
	...
	fd_set fdset;
		...
		switch (flag ? select(FD_SETSIZE, &fdset, NULL, NULL, NULL) : select(fd + 1, NULL, &fdset, NULL, NULL)) {
			case -1: {
				fprintf(stderr, "%d: %s: %s\n",getpid(), program, strerror(errno));
				return -1;
			}
